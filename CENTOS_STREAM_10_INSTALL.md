# Install labctl on CentOS Stream 10

Run these steps on an **x86_64 CentOS Stream 10 host**, using your normal login
...
The host runs CentOS Stream 10; the lab exercises run **Rocky Linux 9**.
...
sudo dnf install \
  ...
  libvirt-client virt-install dnsmasq openssh-clients python3-pip \
  policycoreutils-python-utils polkit git curl xorriso
sudo dnf install epel-release
sudo /usr/bin/crb enable
sudo dnf install pipx
```

EPEL supplies pipx and the CRB helper. Keep package signature checking enabled
and SELinux enforcing.

### Install cloud-localds

The tested CentOS Stream 10 repositories (including CRB and EPEL) do not supply
`cloud-localds`. Install this reviewed, commit-pinned upstream script rather
than assuming Fedora's `cloud-utils` package is available:

```sh
work=$(mktemp -d "$HOME/cloud-localds-review.XXXXXX")
curl --fail --location --show-error --output "$work/cloud-localds" \
  https://raw.githubusercontent.com/canonical/cloud-utils/646ab04dcc275565608af3acc2f27ad8ca79dcfe/bin/cloud-localds
printf '%s  %s\n' \
  936eeaf470b208418689a604a1ae7d0dac3f45ea24d68c2c187eac8957543157 \
  "$work/cloud-localds" | sha256sum --check
```

Continue only if the checksum reports **OK**. Review the script, then install it:

```sh
less "$work/cloud-localds"
sudo install -o root -g root -m 0755 "$work/cloud-localds" /usr/local/bin/cloud-localds
if ! command -v genisoimage >/dev/null 2>&1; then
  test -x /usr/bin/mkisofs && \
    sudo ln -s /usr/bin/mkisofs /usr/local/bin/genisoimage
fi
cloud-localds --help
```

The script expects `genisoimage`; xorriso provides the compatible `mkisofs`
frontend. The conditional creates that name only when missing. This script is
not RPM-managed; maintain it separately. LX001 creation below tests actual
seed-image generation, not just the help command.
...
[CentOS verification](docs/centos-stream-10-verification.md). Installation
