...
LAB_ID = "AN102"
STAGING_DIRECTORY = "/tmp/labctl-an102-grade"  # noqa: S108
KEY_COMMAND = (
    "umask 077; /usr/bin/mkdir -p /home/student/.ssh; "
    "test -f /home/student/.ssh/id_an102_grade || "
    "/usr/bin/ssh-keygen -q -t ed25519 -N '' -f /home/student/.ssh/id_an102_grade"
)
PUBLIC_KEY_COMMAND = "/usr/bin/cat -- /home/student/.ssh/id_an102_grade.pub"
...
CLEANUP_COMMAND = "/usr/bin/rm -rf -- /tmp/labctl-an102-grade"
CHECKS = {
    ...
    "web content is exact": (
        ...
        base64.b64encode(b"Reusable Ansible role\n").decode("ascii"),
    ),
}
...
def main() -> int:
    try:
        ...
        if context.get("schema_version") != 1 or context.get("lab_id") != LAB_ID:
            raise ValueError("grading context does not describe AN102 schema v1")
    ...
    try:
        ...
        inventory = (
            ...
            "ansible_ssh_private_key_file=/home/student/.ssh/id_an102_grade "
            "ansible_ssh_common_args='-o StrictHostKeyChecking=yes "
            "-o UserKnownHostsFile=/tmp/labctl-an102-grade/known_hosts'\n"
        ).encode("ascii")
        ...
        transferred = (
            _remote(
                ...
                _python_write_command(
                    {
                        "/tmp/labctl-an102-grade/inventory.ini": inventory,  # noqa: S108
                        "/tmp/labctl-an102-grade/known_hosts": known_hosts,  # noqa: S108
                        "/tmp/labctl-an102-grade/ansible.cfg": ANSIBLE_CONFIG,  # noqa: S108
                    }
                ),
            )
        ...
        repeated = (
            _remote(ssh, controller, PLAYBOOK_COMMAND)
            if played and played.returncode == 0
            else None
        )
        results.append(
            (
