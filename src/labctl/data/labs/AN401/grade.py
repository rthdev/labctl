...
LAB_ID = "AN401"
STAGING_DIRECTORY = "/tmp/labctl-an401-grade"  # noqa: S108
KEY_PATH = "/home/student/.ssh/id_an401_grade"
KEY_COMMAND = (
...
def _idempotent_recap(output: str, required_hosts: set[str]) -> bool:
    rows = {
    ...
    return set(rows) == required_hosts and all(values == (0, 0, 0) for values in rows.values())
...
def main() -> int:
    ...
    try:
        ...
        if installed is not None and installed.returncode == 0:
            transferred = _remote(
                ...
                _python_write_command(
                    {
                        f"{STAGING_DIRECTORY}/inventory.ini": inventory,
                        f"{STAGING_DIRECTORY}/ansible.cfg": (
                            b"[defaults]\nstdout_callback = default\n"
                        ),
                        f"{STAGING_DIRECTORY}/known_hosts": Path(
        ...
        first = _remote(ssh, controller, PLAYBOOK_COMMAND) if arranged else None
        ...
        results.append(
            (
                "second playbook run reports changed=0 and failures=0",
                bool(second_ok and second and _idempotent_recap(second.stdout, {"target"})),
            )
        )
    finally:
