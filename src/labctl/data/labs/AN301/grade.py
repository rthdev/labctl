...
STAGING = "/tmp/labctl-an301-grade"  # noqa: S108
KEY = "/home/student/.ssh/id_an301_grade"
KEY_COMMAND = (
...
PLAYBOOK_COMMAND = (
    f"/usr/bin/env ANSIBLE_CONFIG={STAGING}/ansible.cfg ANSIBLE_NOCOLOR=1 "
    "ANSIBLE_FORCE_COLOR=0 ANSIBLE_STDOUT_CALLBACK=default ANSIBLE_CALLBACKS_ENABLED= "
...
def _changed_zero(output: str) -> bool:
    if "PLAY RECAP" not in output:
        return False
    recap_output = output.rsplit("PLAY RECAP", 1)[1]
    recap = {
        match.group("host"): int(match.group("changed"))
        for match in re.finditer(
            r"^(?P<host>web[123])\s*:[^\n]*\bchanged=(?P<changed>\d+)\b[^\n]*"
            r"\bunreachable=0\b[^\n]*\bfailed=0\b",
            recap_output,
            re.MULTILINE,
        )
    }
    return recap == {name: 0 for name in TARGETS}
...
def main() -> int:
    ...
    try:
        ...
        first = _remote(ssh, hosts["controller"], PLAYBOOK_COMMAND) if staged else None
        first_ok = bool(first and first.returncode == 0)
        results.append(("controller playbook completed successfully", first_ok))
        second = _remote(ssh, hosts["controller"], PLAYBOOK_COMMAND) if first_ok else None
        results.append(
            (
                "second playbook run changed zero hosts",
                bool(second and second.returncode == 0 and _changed_zero(second.stdout)),
            )
        )
        ...
        for name in TARGETS:
            ...
            response = _remote(
                ssh, hosts[name], "/usr/bin/curl -fsS --max-time 5 http://127.0.0.1/"
            )
            results.append(
                (
            ...
            results.append(
                (
                    f"{name} serves version 3.1.0",
                    response.returncode == 0 and response.stdout.strip() == "AN301 version 3.1.0",
                )
            )
    finally:
