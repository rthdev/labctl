...
LAB_ID = "AN101"
STAGING_DIRECTORY = "/tmp/labctl-an101-grade"  # noqa: S108
KEY_COMMAND = (
    "umask 077; /usr/bin/mkdir -p /home/student/.ssh; "
    "test -f /home/student/.ssh/id_an101_grade || "
    "/usr/bin/ssh-keygen -q -t ed25519 -N '' -f /home/student/.ssh/id_an101_grade"
)
PUBLIC_KEY_COMMAND = "/usr/bin/cat -- /home/student/.ssh/id_an101_grade.pub"
...
CLEANUP_COMMAND = "/usr/bin/rm -rf -- /tmp/labctl-an101-grade"
PID_COMMAND = "/usr/bin/systemctl show -p MainPID --value chronyd"
CHECKS = {
    "chronyd is enabled": ("/usr/bin/systemctl is-enabled chronyd", "enabled"),
    "chronyd is active": ("/usr/bin/systemctl is-active chronyd", "active"),
    "chrony configuration ownership and mode": (
        "/usr/bin/stat -c '%U:%G %a' /etc/chrony.conf",
        "root:root 644",
    ),
    "chrony configuration is exact": (
        "/usr/bin/base64 -w0 -- /etc/chrony.conf",
        base64.b64encode(
            b"pool 2.pool.ntp.org iburst\n"
...
def _recap_reports_no_changes(output: str) -> bool:
    markers = list(re.finditer(r"(?m)^PLAY RECAP \*+\s*$", output))
    if not markers:
        return False
    recap = output[markers[-1].end() :]
    lines = re.findall(r"(?m)^\s*target\s*:\s+([^\r\n]+?)\s*$", recap)
    if len(lines) != 1:
        return False
    tokens = lines[0].split()
    if not tokens or any(not re.fullmatch(r"[a-z_]+=\d+", token) for token in tokens):
        return False
    stats = dict(token.split("=", 1) for token in tokens)
    return all(stats.get(field) == "0" for field in ("changed", "failed", "unreachable"))
...
def main() -> int:
    try:
        ...
        if context.get("schema_version") != 1 or context.get("lab_id") != LAB_ID:
            raise ValueError("grading context does not describe AN101 schema v1")
    ...
    try:
        ...
        inventory = (
            ...
            "ansible_ssh_private_key_file=/home/student/.ssh/id_an101_grade "
            "ansible_ssh_common_args='-o StrictHostKeyChecking=yes "
            "-o UserKnownHostsFile=/tmp/labctl-an101-grade/known_hosts'\n"
        ).encode("ascii")
        ...
        transferred = (
            _remote(
                ...
                _python_write_command(
                    {
                        "/tmp/labctl-an101-grade/inventory.ini": inventory,  # noqa: S108
                        "/tmp/labctl-an101-grade/known_hosts": known_hosts,  # noqa: S108
                        "/tmp/labctl-an101-grade/ansible.cfg": ANSIBLE_CONFIG,  # noqa: S108
                    }
                ),
            )
        ...
        results.append(
            ("first controller playbook run completed", bool(played and played.returncode == 0))
        )
        before = _remote(ssh, target, PID_COMMAND) if played and played.returncode == 0 else None
        repeated = _remote(ssh, controller, PLAYBOOK_COMMAND) if before else None
        results.append(
            (
                "second controller playbook run completed",
                bool(repeated and repeated.returncode == 0),
            )
        )
        after = _remote(ssh, target, PID_COMMAND) if repeated and repeated.returncode == 0 else None
        before_pid = before.stdout.strip() if before and before.returncode == 0 else ""
        after_pid = after.stdout.strip() if after and after.returncode == 0 else ""
        recap = repeated.stdout if repeated and repeated.returncode == 0 else ""
        results.append(
            (
                "second run reports changed=0 for target",
                _recap_reports_no_changes(recap),
            )
        )
        results.append(
            (
                "unchanged second run does not restart chronyd",
                before_pid.isdecimal() and before_pid != "0" and after_pid == before_pid,
            )
        )
    finally:
