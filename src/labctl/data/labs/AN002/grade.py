...
LAB_ID = "AN002"
STAGING_DIRECTORY = "/tmp/labctl-an002-grade"  # noqa: S108
KEY_COMMAND = (
    "umask 077; /usr/bin/mkdir -p /home/student/.ssh; "
    "test -f /home/student/.ssh/id_an002_grade || "
    "/usr/bin/ssh-keygen -q -t ed25519 -N '' -f /home/student/.ssh/id_an002_grade"
)
PUBLIC_KEY_COMMAND = "/usr/bin/cat -- /home/student/.ssh/id_an002_grade.pub"
DEPLOY_PUBLIC_KEY_COMMAND = "/usr/bin/cat -- /home/student/ansible-lab/deploy_access_key.pub"
PLAYBOOK_COMMAND = (
    "/usr/bin/ansible-playbook -i /tmp/labctl-an002-grade/inventory.ini "
    "/home/student/ansible-lab/site.yml"
)
CLEANUP_COMMAND = "/usr/bin/rm -rf -- /tmp/labctl-an002-grade"
CHECKS = {
    "automation group exists": ("/usr/bin/getent group automation", None),
    "deploy belongs to automation": ("/usr/bin/id -nG deploy", "automation"),
    "auditor belongs to automation": ("/usr/bin/id -nG auditor", "automation"),
    "shared path is a directory": ("/usr/bin/test -d -- /srv/automation", None),
    "shared directory has setgid ownership and mode": (
        "/usr/bin/stat -c '%U:%G %a' /srv/automation",
        "root:automation 2770",
    ),
    "deploy authorized_keys ownership and mode": (
        "/usr/bin/stat -c '%U:%G %a' /home/deploy/.ssh/authorized_keys",
        "deploy:deploy 600",
    ),
}
...
def main() -> int:
    try:
        ...
        if context.get("schema_version") != 1 or context.get("lab_id") != LAB_ID:
            raise ValueError("grading context does not describe AN002 schema v1")
    ...
    try:
        ...
        inventory = (
            ...
            "ansible_ssh_private_key_file=/home/student/.ssh/id_an002_grade "
            "ansible_ssh_common_args='-o StrictHostKeyChecking=yes "
            "-o UserKnownHostsFile=/tmp/labctl-an002-grade/known_hosts'\n"
        ).encode("ascii")
        ...
        transferred = (
            _remote(
                ...
                _python_write_command(
                    {
                        "/tmp/labctl-an002-grade/inventory.ini": inventory,  # noqa: S108
                        "/tmp/labctl-an002-grade/known_hosts": known_hosts,  # noqa: S108
                    }
                ),
            )
    ...
    for description, (command, expected) in CHECKS.items():
        ...
        if description.endswith("belongs to automation"):
            passed = checked.returncode == 0 and "automation" in actual.split()
        elif expected is None:
            passed = checked.returncode == 0
        else:
            passed = checked.returncode == 0 and actual == expected
        results.append((description, passed))

    expected_key = _remote(ssh, controller, DEPLOY_PUBLIC_KEY_COMMAND)
    authorized = _remote(ssh, target, "/usr/bin/base64 -w0 -- /home/deploy/.ssh/authorized_keys")
    expected_authorized = base64.b64encode(
        (expected_key.stdout.strip() + "\n").encode("ascii")
    ).decode("ascii")
    results.append(
        (
            "deploy authorized key is exact",
            expected_key.returncode == 0
            and bool(_PUBLIC_KEY.fullmatch(expected_key.stdout.strip()))
            and authorized.returncode == 0
            and authorized.stdout.strip() == expected_authorized,
        )
    )
